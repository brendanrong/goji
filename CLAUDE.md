...
- `Transcriber.swift`: multi-engine wrapper (actor). Parakeet v2/v3 via AsrManager, Cohere Transcribe via CoherePipeline (pinned to English for now); loads whichever SettingsStore.selectedModel says. An acoustic Names & phrases boost (CTC word spotting + rescoring) was built and PARKED during v1.0.15: the library's batch VocabularyRescorer misaligns replacements onto neighboring words, and a custom confirmation step missed true positives. Vocabulary corrections run through the Cleaner prompt (with learned aliases) instead. Do not revive the acoustic path without an offline test harness of recorded audio; see git history.
...
- `HistoryStore.swift`: recent transcripts, capped at 50, local UserDefaults only. Rows are editable in the History pane ("fix and learn").
- `CorrectionLearner.swift`: word-level LCS diff between a transcript and the user's fix; changed pairs feed SettingsStore.learn (right spelling into Names & phrases, mishearing stored as its alias, aliases flow into boosting and the cleanup prompt).
