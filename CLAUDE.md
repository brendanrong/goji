...
- `Formatter.swift`: TranscriptFormatter, deterministic cleanup on every dictation (spoken commands "new line" / "new paragraph" / "scratch that", fillers, stutters). Foundation-only on purpose; `scripts/formatter-tests.sh` compiles it standalone against a fixture table. Runs BEFORE the AI Cleaner, which no longer owns commands or line breaks (it dropped them in testing) and cleans paragraph by paragraph.
